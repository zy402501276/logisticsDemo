 ...
 */
class GiixAllCode extends ModelCode {
	...
	//model基类
	public $baseClass = 'BaseModel';
	//model生成路径
	public $modelPath = 'application.model';
	//form生成路径
	public $formPath = 'application.form';
	//form基类
	public $baseFormClass = 'BaseForm';
	...
	public function prepare() {
		...
		foreach ($tables as $table) {
			...
			$params = array(
				...
				'formClass' => $className.'BaseForm',
				'baseFormClass' => $this->baseFormClass,
				'baseModelClass' => $this->baseClass,
				'pk' => $pk
			);
			...
			//生成modle
			!defined('STANDALONE') AND define('STANDALONE', 'base');
			if (!file_exists(Yii::getPathOfAlias($this->modelPath.'/'.STANDALONE.'/'.$className) . '.php')) {
				$this->files[] = new CCodeFile(
					Yii::getPathOfAlias($this->modelPath.'/'.STANDALONE.'/'.$className) . '.php',
					$this->render($templatePath . DIRECTORY_SEPARATOR . 'model.php', $params)
				);
			}
			//生成form
			$fileName = 'BaseForm.php';
			if (file_exists(Yii::getPathOfAlias($this->formPath.'/'.STANDALONE.'/'.$className).$fileName)) {
				$fileName = 'BaseForm.php.1';
			}
			$this->files[] = new CCodeFile(
				Yii::getPathOfAlias($this->formPath.'/'.STANDALONE.'/'.$className).$fileName,
				$this->render($templatePath . DIRECTORY_SEPARATOR . 'form.php', $params)
			);
		}
	}
	...
	public function generateLabelsEx($table, $className) {
		...
		foreach ($table->columns as $column) {
			if ($column->isForeignKey) {
				...
				$label = empty($column->comment) ? $label : $column->comment; 
